...
export default function BoughtGiftCodeItem({ code }: { code: string }) {
  ...
  return (
    <button
      className={
        "flex w-full cursor-pointer flex-row gap-[1.163vw] lg:!gap-[0.26vw] hover:opacity-80"
      }
      onClick={() => copyCodes(code)}
    >
      <div
        className={
          "w-full rounded-[1.163vw] lg:!rounded-[0.26vw] border border-foreground p-[1.163vw] lg:!p-[0.26vw] text-start font-plexsans text-[3.256vw] lg:!text-[0.729vw]"
        }
      >
        {code}
      </div>
      <AnimatePresence>
        {linkCopied ? (
          <div
            className={
              "flex items-center justify-center rounded-[1.163vw] lg:!rounded-[0.26vw] border border-foreground p-[1.163vw] lg:!p-[0.26vw]"
            }
          >
            <motion.svg
              aria-hidden="true"
              ...
              viewBox="0 0 17 18"
              className={
                "h-[4.419vw] lg:!h-[1.25vw] w-[4.419vw] lg:!w-[1.25vw]"
              }
            >
              <motion.polyline
                fill="none"
